...
class ReportGenerator extends Common
{
     ...
     * table  */
    const REPORT_ID_REGEX = '/^\d+-(?(?=autogenerated)autogenerated-[a-z]+|[0-9\.]+)$/';
    const REPORT_DATE_REGEX =  '/^\d{4}(-\d{2}){2}$/';
    ...
    const REPORT_CHART_REF_REGEX = '/^\d+(-(?(?=autogenerated)autogenerated-[a-z]+|[0-9\.]+))?;\d+$/';
    const REPORT_CHART_DID_REGEX = '/^_d\d+$/';

    /* the save_report controller use a custom data serialization for charts
     * that have been modified from the original report */
    const CHART_CACHEREF_REGEX = '/^(\d{4}(-\d{2}){2};){2}(?(?=xd_report_volatile_)xd_report_volatile_\d+;\d+(_d\d+)?|\d+-(?(?=autogenerated)autogenerated-[a-z]+|[0-9\.]+);\d+)$/';
     ...
     */
    const REPORT_TMPDIR_REGEX = '/^(\d+-(autogenerated-[a-z]+|[0-9\.]+)|monthly_compliance_report)-[a-zA-Z0-9\.]+$/';
