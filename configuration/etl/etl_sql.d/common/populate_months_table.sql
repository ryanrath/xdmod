...
WITH RECURSIVE months(id, year, date, month, month_start, month_end, hours, seconds, month_start_ts,
                      month_end_ts,
                      month_middle_ts) AS (SELECT
                                               2001 * 100000 + 1                                as id, 2001 as year,
                                               DATE('2001-01-01')                               as date, 1 as month,
                                           ...
                                           UNION ALL
                                           SELECT
                                                       EXTRACT(YEAR FROM DATE_ADD(date, INTERVAL 1 MONTH)) * 100000 +
                                                       EXTRACT(MONTH FROM DATE_ADD(date, INTERVAL 1 MONTH))                             as id,
                                                       ...
                                                       CONCAT(
                                                           ...
                                                           '-', timestampdiff(day, concat(
                                                           year(DATE_ADD(date, INTERVAL 1 MONTH)), '-',
                                                           month(DATE_ADD(date, INTERVAL 1 MONTH)), '-01'), date_add(
                                                                                  concat(
                                                                                      year(DATE_ADD(date, INTERVAL 1 MONTH)),
                                                                                      '-',
                                                       ...
                                                       UNIX_TIMESTAMP(CONCAT(
                                                           ...
                                                           '-', timestampdiff(day, concat(
                                                               year(DATE_ADD(date, INTERVAL 1 MONTH)), '-',
                                                               month(DATE_ADD(date, INTERVAL 1 MONTH)), '-01'),
                                                                              date_add(concat(
                                                                                           year(DATE_ADD(date, INTERVAL 1 MONTH)),
                                                                                           '-',
                                                                                           month(DATE_ADD(date, INTERVAL 1 MONTH)),
                                                       ...
                                                       UNIX_TIMESTAMP(CONCAT(
                                                           EXTRACT(YEAR FROM DATE_ADD(date, INTERVAL 1 MONTH)), /* YEAR */
                                                           '-', EXTRACT(MONTH FROM DATE_ADD(date, INTERVAL 1 MONTH)),/* MONTH */
                                                           '-', FLOOR(timestampdiff(day, concat(
                                                               year(DATE_ADD(date, INTERVAL 1 MONTH)), '-',
                                                               month(DATE_ADD(date, INTERVAL 1 MONTH)), '-01'),
                                                                                    date_add(concat(
                                                                                                 year(DATE_ADD(date, INTERVAL 1 MONTH)),
