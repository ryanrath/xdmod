...
SELECT
    ...
    timestampdiff(day,
                  concat(year(DATE('2000-01-01')), '-', month(DATE('2000-01-01')), '-01'),
                  date_add(concat(year(DATE('2000-01-01')), '-', month(DATE('2000-01-01')), '-01'), interval 1 quarter)
    ) * 24                                           as hours,
    timestampdiff(day,
                  concat(year(DATE('2000-01-01')), '-', month(DATE('2000-01-01')), '-01'),
                  date_add(concat(year(DATE('2000-01-01')), '-', month(DATE('2000-01-01')), '-01'), interval 1 quarter)
...
UNION ALL
SELECT
    EXTRACT(YEAR FROM DATE_ADD(date, INTERVAL 1 QUARTER)) * 100000 + EXTRACT(QUARTER FROM DATE_ADD(date, INTERVAL 1 QUARTER)) as id,
    ...
    timestampdiff(
        day,
        concat(year(DATE_ADD(date, INTERVAL 1 QUARTER)), '-', month(DATE_ADD(date, INTERVAL 1 QUARTER)), '-01'),
               date_add(concat(year(DATE_ADD(date, INTERVAL 1 QUARTER)), '-', month(DATE_ADD(date, INTERVAL 1 QUARTER)), '-01'), interval 1 QUARTER)
    ) * 24                                                                                                                    as hours,
    timestampdiff(
        day,
        concat(year(DATE_ADD(date, INTERVAL 1 QUARTER)), '-', month(DATE_ADD(date, INTERVAL 1 QUARTER)), '-01'),
    ...
    UNIX_TIMESTAMP(CONCAT(DATE(DATE_ADD(DATE_ADD(date, INTERVAL 1 QUARTER),
                                        INTERVAL timestampdiff(
                                                     day,
                                                     DATE(concat(year(DATE_ADD(date, INTERVAL 1 quarter)), '-',
                                                                 month(DATE_ADD(date, INTERVAL 1 quarter)),
                                                                 '-01')) /* beginning date */,
                                                     date_add(concat(year(DATE_ADD(date, INTERVAL 1 quarter)), '-',
                                                                     month(DATE_ADD(date, INTERVAL 1 quarter)),
    ...
    UNIX_TIMESTAMP(TIMESTAMP(CONCAT(
        DATE_ADD(DATE(CONCAT(
            DATE_ADD(date, INTERVAL 1 QUARTER),
            ' 00:00:00'))/* START DATE */,
        INTERVAL FLOOR(timestampdiff(
                           day,
                           DATE(concat(year(DATE_ADD(date, INTERVAL 1 QUARTER)), '-',
                                       month(DATE_ADD(date, INTERVAL 1 QUARTER)),
                                       '-01')),
    ...
    )/* FLOOR(Number of days in the quarter / 2) */                                                                           as quarter_middle_ts
FROM quarters
